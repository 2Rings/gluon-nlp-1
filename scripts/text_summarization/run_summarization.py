...
def run_train():
    ...
    for epoch_id in range(args.epochs):
        ...
        with mx.autograd.record():
            ...
            #decoder_outputs: list of decoder_output with shape(batch_size, 2*hidden_dim), len(decoder_outputs) = abs_length - 1
            decoder_outputs = model(art_seq, abs_seq[:, :-1], art_valid_length, abs_valid_length-1)
            #covert decoder_outputs to ndarray so that it can be feed into hybrid_forward(symbol)
